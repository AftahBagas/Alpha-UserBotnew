...
@register(outgoing=True, pattern=r"^\.notifof$")
...
@register(outgoing=True, pattern=r"^\.notifonn$")
...
@register(outgoing=True, pattern=r"^\.(?:setujuu|okk)\s?(.)?")
...
@register(outgoing=True, pattern=r"^\.(?:tolakk|nopmm)\s?(.)?")
...
@register(outgoing=True, pattern=r"^\.blokir$")
...
@register(outgoing=True, pattern=r"^\.unblokir$")
...
@register(outgoing=True, pattern=r"^.(sett|gett|resett) pm_msgg(?: |$)(\w*)")
async def add_pmsg(cust_msg):
    """Set your own Unapproved message"""
    if not PM_AUTO_BAN:
        return await cust_msg.edit("**Alpha User Anda Harus Menyetel** `PM_AUTO_BAN` **Ke** `True`")
    try:
