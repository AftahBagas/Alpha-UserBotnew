...
if Var.BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query.startswith("Userbot"):
            ...
            result = builder.article(
                "© Userbot Help",
                text="{}\nDATA IN MY MASTER'S SERVER: {}".format(query, len(CMD_HELP)),
                buttons=buttons,
                link_preview=False,
            )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = f"Buat Sendiri Userbotmu Jangan Menggunakan Milik {ALIVE_NAME}"
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = f"Buat Userbotmu Sendiri Jangan Menggunakan Milik {ALIVE_NAME}"
            await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
