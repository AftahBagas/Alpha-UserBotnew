...
async def _(event):
    if not event.reply_to_msg_id:
        await event.edit("`Alpha User Mohon Balas Ke Media Apapun`")
        return
    reply_message = await event.get_reply_message()
    if not reply_message.media:
        await event.edit("`Alpha User Mohon Balas Ke Gambar/Sticker/Video`")
        return
...
CMD_HELP.update({"wordcloud": "📚 **Cmd** : `.wc`\n"
                 "📄 **Descriptions** : membuat seni wordcloud dari media\n\n"})
