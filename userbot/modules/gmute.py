                ...
                if i.sender == str(guser.id):
                    ...
                    if admin or creator:
                        try:
                            ...
                            await tele.reply(
                                f"**Alpha, Pengguna Gmute Telah Bergabung** \n"
...
async def gben(userbot):
    ...
    if not sender.id == me.id:
        dark = await dc.reply("`Alpha Ingin Mengaktifkan Perintah Global Mute!`")
    else:
        dark = await dc.edit("`Memproses Global Banned Pengguna Ini :)`")
    me = await userbot.client.get_me()
    await dark.edit(f"`Global Mute Akan Segera Aktif, Anda Akan Mute Secara Global Oleh Alpha :)`")
...
async def gunben(userbot):
    ...
    me = await userbot.client.get_me()
    await dark.edit(f"`Memulai Membatalkan Perintah Global Mute, Pengguna Ini Akan Dapat Bergabung Ke Grup Anda ヅ`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await userbot.get_chat()
    a = b = 0
...
\n**Penjelasan:** Melakukan Mute Secara Global Ke Semua Grup Dimana Alpha Sebagai Admin.\
\n\n**Perintah:** `.ungmuted`\
\n**Penjelasan:** Membatalkan Global Mute"})
