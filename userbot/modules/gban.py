...
async def gspide(rk):
    ...
    if not sender.id == me.id:
        rkp = await lazy.reply("`Memproses Gban...`")
    else:
        rkp = await lazy.edit("`Memproses Gban...`")
    me = await rk.client.get_me()
    await rkp.edit(f"`{ALIVE_NAME}:` **Meminta untuk mengban pengguna!**")
    ...
    if user:
        if user.id == 1743866353:
            return await rkp.edit(f"`{ALIVE_NAME}:`**Anda Sepertinya Tidak Bisa Gban Pengguna Ini , Karena Dia Adalah Pembuat Saya 😈**")
        try:
