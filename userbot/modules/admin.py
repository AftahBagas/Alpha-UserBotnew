...
NO_ADMIN = "`Alpha Bukan Bagian Admin:)`"
NO_PERM = "`Alpha Tidak Mempunyai Izin!`"
NO_SQL = "`Berjalan Pada Mode Non-SQL`"

CHAT_PP_CHANGED = "`Alpha Berhasil Mengubah Profil Grup`"
CHAT_PP_ERROR = (
    "`Ada Masalah Dengan Memperbarui Foto,`"
    "`Mungkin Karna Alpha Bukan Admin,`"
...
async def promote(promt):
    ...
    await promt.edit("`Mengangkat Pengguna Sebagai Admin... Mohon Menunggu`")
    ...
    try:
        ...
        await promt.edit("`Berhasil Mengangkat Pengguna Ini Sebagai Admin!`")
        await sleep(5)
        await promt.delete()
...
async def ban(bon):
    ...
    if reason:
        await bon.edit(
            f"`PENGGUNA:` [{user.first_name}](tg://user?id={user.id})\n`ID:` `{str(user.id)}` Dibanned Lord Java !!\n`Karena:` {reason}"
        )
    else:
        await bon.edit(
            f"`PENGGUNA:` [{user.first_name}](tg://user?id={user.id})\n`ID:` `{str(user.id)}` Dibanned Lord Java !"
...
async def nothanos(unbon):
    ...
    await unbon.edit("`Sedang Menjalankan Unban...`")
...
async def spider(spdr):
    ...
    if mute(spdr.chat_id, user.id) is False:
        return await spdr.edit("`Error! Penggunaan Sampun Dimute.`")
    else:
        try:
...
async def ungmoot(un_gmute):
    ...
    await un_gmute.edit("```Menjalankan Global Mute ! ```")
...
async def gspider(gspdr):
    ...
    if gmute(user.id) is False:
        ...
        if reason:
            await gspdr.edit(f"**Dibisukan Disemua Media Lord Java!**\n**Alasan:** `{reason}`")
        else:
...
async def rm_deletedacc(show):
    ...
    if con != "clean":
        await show.edit("`Mencari Akun Yang Tidak Aktif...`")
        ...
        if del_u > 0:
            del_status = (
                f"`Menemukan` **{del_u}** `Akun Tidak Aktif Dalam Grup Ini,"
    ...
    # Well
    if not admin and not creator:
        return await show.edit("`Alpha Bukan Admin Disini!`")
    ...
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                return await show.edit("`Alpha Tidak Memiliki Izin Untuk Membanned Disini`")
...
async def get_admin(show):
    ...
    mentions = f"<b>👮 List Daftar Admin Group {title}:</b> \n"
    try:
...
async def get_users(show):
    ...
    try:
        ...
        await show.edit("Alpha, Grup Ini Terlalu Besar Mengunggah Daftar Pengguna Sebagai File.")
...
async def get_usersdel(show):
    ...
    try:
        ...
        await show.edit(
            "Alpha,Group Ini Terlalu Besar Untuk Mengunggah File Akun Terhapus."
        )
        file = open("daftarpengguna.txt", "w+")
        file.write(mentions)
        file.close()
