...
NO_ADMIN = "`Lord Java Bukan Bagian Admin:)`"
NO_PERM = "`Lord Java Tidak Mempunyai Izin!`"
NO_SQL = "`Berjalan Pada Mode Non-SQL`"

CHAT_PP_CHANGED = "`Lord Java Berhasil Mengubah Profil Grup`"
CHAT_PP_ERROR = (
...
async def set_group_photo(gpic):
    if not gpic.is_group:
        await gpic.edit("`Lord Java Mohon Jalankan Perintah Ini Di Grup.`")
        return
    ...
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                return await show.edit("`Lord Java Tidak Memiliki Izin Untuk Membanned Disini`")
...
async def get_usersdel(show):
    ...
    try:
        ...
        await show.edit(
            "Lord Java,Group Ini Terlalu Besar Untuk Mengunggah File Akun Terhapus."
        )
        file = open("daftarpengguna.txt", "w+")
        file.write(mentions)
        file.close()
