...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"✨ᴘᴇsᴀɴ ᴏᴛᴏᴍᴀᴛɪs\n╭╼═══╾╼═══╾╼═══╾╼═━✥ \n┣ {ALIVE_NAME} ᴀꜰᴋ ᴅᴜʟᴜ\n┣ ᴀʟᴀꜱᴀɴ : {AFKREASON}\n╰╼═══╾╼═══╾╼═━✥")
    else:
        await afk_e.edit("✨ ʟᴏʀᴅ ᴊᴀᴠᴀ ᴀғᴋ\n╭╼═══╾╼═══╾╼═━✥ \n┣ ᴍʙᴏᴛᴇɴ ᴘᴀʀᴇɴɢ ᴋᴀɴɢᴇɴ 🌹 \n╰╼═══╾╼══━✥")
    if user.last_name:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name=user.last_name + "‹ᴀғᴋ›"))
    else:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name="‹ᴀғᴋ›"))
    if BOTLOG:
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**LORD JAVA ᴛᴇʟᴀʜ ᴋᴇᴍʙᴀʟɪ! ᴇᴍᴀɴɢ ᴀᴅᴀ ʏᴀɴɢ ʀɪɴᴅᴜ?**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"✨ ʟᴏʀᴅ ᴊᴀᴠᴀ ᴀғᴋ\n╭╼═══╾╼═══╾╼═══╾╼═━✥ \n┣ {ALIVE_NAME} ꜱᴇᴅᴀɴɢ ᴀꜰᴋ\n┣ ꜱᴇᴊᴀᴋ: {afk_since}\n┣ ᴀʟᴀꜱᴀɴ: {AFKREASON}\n╰╼═══╾╼══╾╼══━✥")
                else:
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"✨ ʟᴏʀᴅ ᴊᴀᴠᴀ ᴀғᴋ\n╭╼═══╾╼═══╾╼═══╾╼═━✥ \n┣ {ALIVE_NAME} ꜱᴇᴅᴀɴɢ ᴀꜰᴋ\n┣ ꜱᴇᴊᴀᴋ: {afk_since}\n┣ ᴀʟᴀꜱᴀɴ: {AFKREASON}\n╰╼═══╾╼══╾╼══━✥")
                else:
